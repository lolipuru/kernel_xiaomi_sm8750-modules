...
#define DP_3NM_C_READY		BIT(0)
#define DP_3NM_FREQ_DONE	BIT(0)
#define DP_3NM_PLL_LOCKED	BIT(1)
#define DP_3NM_PHY_READY	BIT(1)
...
{
	u32 div, val;
	unsigned long clk0_rate, clk1_rate;
	...
	clk0_rate = pll->vco_rate;
	clk1_rate = pll->vco_rate;
	do_div(clk0_rate, 10);
	do_div(clk1_rate, div);
	 ...
	 */
	clk_set_rate(pll->clk_data->clks[0], clk0_rate);
	clk_set_rate(pll->clk_data->clks[1], clk1_rate);

	DP_DEBUG("val=%#x div=%x link_clk rate=%lu vco_div_clk rate=%lu\n",
			val, div, clk0_rate, clk1_rate);
		...
		unsigned long rate)
{
	...
	rc = dp_vco_pll_init_db_3nm(pdb, rate);
	if (rc < 0) {
		...
		enum dp_3nm_pll_status status)
{
	...
	case FREQ_DONE:
		...
		bit = DP_3NM_FREQ_DONE;
		break;
	case PLL_LOCKED:
		base = dp_pll_get_base(dp_pll);
		reg = QSERDES_COM_CMN_STATUS;
		bit = DP_3NM_PLL_LOCKED;
		break;
	case PHY_READY:
		base = dp_pll_get_base(dp_phy);
		reg = DP_PHY_STATUS;
		bit = DP_3NM_PHY_READY;
		break;
	case TSYNC_DONE:
		base = dp_pll_get_base(dp_phy);
		reg = DP_PHY_STATUS;
		bit = DP_3NM_TSYNC_DONE;
		break;
	default:
		return false;
	}
...
static int dp_pll_enable_3nm(struct dp_pll *pll)
{
	int rc = 0;
	...
	if (!dp_3nm_pll_get_status(pll, C_READY)) {
		rc = -EINVAL;
		goto lock_err;
	}

	if (!dp_3nm_pll_get_status(pll, FREQ_DONE)) {
		rc = -EINVAL;
		goto lock_err;
	}

	if (!dp_3nm_pll_get_status(pll, PLL_LOCKED)) {
		rc = -EINVAL;
		goto lock_err;
	}
	...
	if (!dp_3nm_pll_get_status(pll, TSYNC_DONE)) {
		rc = -EINVAL;
		goto lock_err;
	}

	if (!dp_3nm_pll_get_status(pll, PHY_READY)) {
		rc = -EINVAL;
		goto lock_err;
	}
...
static void dp_pll_disable_3nm(struct dp_pll *pll)
{
...
static int dp_vco_set_rate_3nm(struct dp_pll *pll, unsigned long rate)
{
	int rc = 0;
	...
	rc = dp_config_vco_rate_3nm(pll, rate);
	if (rc < 0) {
...
static int dp_regulator_enable_3nm(struct dp_parser *parser,
...
static int dp_pll_configure(struct dp_pll *pll, unsigned long rate)
{
	...
	pll->vco_rate = rate;
	rc = dp_vco_set_rate_3nm(pll, rate);
	if (rc < 0) {
...
static int dp_pll_prepare(struct dp_pll *pll)
{
	...
	/*
	 * Enable DP_PM_PLL regulator if the PLL revision is 3nm-V1 and the
						 ...
						 unsigned long parent_rate)
{
	...
	rate = mult_frac(pll->vco_rate, pll->clk_factor, 10);
...
static unsigned long dp_pll_vco_div_clk_get_rate(struct dp_pll *pll)
{
	if (pll->vco_rate == DP_VCO_HSCLK_RATE_8100MHZDIV1000)
		return (mult_frac(pll->vco_rate, pll->clk_factor, 6));
	else if (pll->vco_rate == DP_VCO_HSCLK_RATE_5400MHZDIV1000)
		return (mult_frac(pll->vco_rate, pll->clk_factor, 4));
	else
		return (mult_frac(pll->vco_rate, pll->clk_factor, 2));
}
